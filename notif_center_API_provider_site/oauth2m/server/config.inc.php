 ...
 */
header('X-XRDS-Location: http://notifcenter.zapto.org/notifcenter/oauth2m/server/services.xrds.php');
...
function assert_logged_in()
{
	...
	{
		$uri = $_SESSION['toktok'] = $_SERVER['REQUEST_URI'];
		header('Location: http://notifcenter.zapto.org/notifcenter/oauth2m/server/logon.php?goto=' . urlencode($uri));
		exit();
	}else{
		$_SESSION['toktok'] = $_SERVER['REQUEST_URI'];
	}
}
