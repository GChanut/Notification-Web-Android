			...
			if(!empty($_SESSION['id'])){
					...
					if(isset($_GET['app'])){
							...
							if($_GET['app'] == 'Facebook'){						
								...
								$str_data = file_get_contents("oauth2m/api/json/".$_SESSION['user'].".json");//Recuperation des données utilisateurs notifcenter
							...
							//Si c'est toutees les autres applications externes
								$str_data = file_get_contents("oauth2m/api/json/".$_SESSION['user'].".json");
								$data = json_decode($str_data,true);
								 for ($i = 0, $len = count($data["application"]); $i < $len; ++$i) {
									if($data["application"][$i]["application_name"] == $_GET['app']){								
										$user = $data["application"][$i]["application_user"];
										$temp = $i;
										$notif = $data["application"][$i]["notification"];
									}
								}
								echo '<table align="center" border="1">'.$notif.'</table>';
							}
...
?>
									<a class="dock-item2" href="javascript:;" onClick="getAppli('<?php echo $appName; ?>');" ><span><?php echo $appName;?></span><img src="img/<?php echo "app$i";?>.png"/></a>
<?php
								}	
							}
						}
